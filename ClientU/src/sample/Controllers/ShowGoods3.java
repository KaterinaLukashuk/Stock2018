...
public class ShowGoods3 {
    ...
    @FXML
    private TableView<Accessor> goodsTable;
...
//
    ...
    void initialize() {
        ...
        ObservableList<Accessor> data = FXCollections.observableArrayList();
        ArrayList<Accessor> mas = null;
        // ArrayList<Product> mas = null;
        try {
            mas = (ArrayList<Accessor>) Main.client.RecvMess();
        ...
        filtr.setOnAction(event -> {
            ...
            {    if (minPrice.getText().isEmpty() || maxPrice.getText().isEmpty())
                ...
                if (combo.getSelectionModel().getSelectedItem().equals("По цене"))
                    Main.client.setMessage("FiltrGoodsByPr3");
                else  Main.client.setMessage("FiltrGoodsByAm3");
                Main.client.SendMess();
                Main.client.setMessage(Integer.parseInt(minPrice.getText()));
                Main.client.SendMess();
                Main.client.setMessage(Integer.parseInt(maxPrice.getText()));
                Main.client.SendMess();
                ObservableList<Accessor> data1 = FXCollections.observableArrayList();
                ArrayList<Accessor> mas1 = null;
                // ArrayList<Product> mas = null;
                try {
                    mas1 = (ArrayList<Accessor>) Main.client.RecvMess();
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                }
        ...
        filtr1.setOnAction(event -> {
            Main.client.setMessage("ShowAllProd3");
            Main.client.SendMess();
            ObservableList<Accessor> data2 = FXCollections.observableArrayList();
            ArrayList<Accessor> mas2 = null;
            // ArrayList<Product> mas = null;
            try {
                mas2 = (ArrayList<Accessor>) Main.client.RecvMess();
            } catch (ClassNotFoundException e) {
                e.printStackTrace();
            }
